...
public class AutoFillListLinearLayout extends LinearLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (mState == STATE_EMPTY) {
            super.onMeasure(
                    MeasureSpec.makeMeasureSpec(getResources().getDisplayMetrics().widthPixels, MeasureSpec.EXACTLY),
        ...
        if (firstVisiablePosition > headViewCount) {
            ...
            // if(false){
            // 是否忽略headView高度
            // childStartPosition = headViewCount - firstVisiablePosition > 0 ?
            // headViewCount - firstVisiablePosition : 0;//忽略HeadView的高度
            // }
