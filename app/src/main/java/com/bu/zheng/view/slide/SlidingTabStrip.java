...
class SlidingTabStrip extends LinearLayout {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (childCount > 0) {
            ...
            int top = height - mSelectedIndicatorThickness;
            //canvas.drawRect(left, top, right, height, mSelectedIndicatorPaint);

            mPath = new Path();
            mPath.moveTo(left + 10 * mDensity, top);
            mPath.lineTo(right - 10 * mDensity, top);
            mPath.lineTo(right - 9 * mDensity, height);
            mPath.lineTo(left + 9 * mDensity, height);
            mPath.close();
